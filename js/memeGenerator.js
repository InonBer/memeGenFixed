...
function renderImgs(imgs = getgImgs()) {
    ...
    imgs.forEach((img, idx) => {
        strHTML += `<img class="galery-img" data-id="${img.id}" src="${img.url}" alt="" onclick="drawImg(this)">`
    });
...
function renderSearches() {
    ...
    var keys = getgKeys();
    console.log(keys);
    var keyMap = getGlobalSearches();
    for (var i = gCurrSerachNum; i < gCurrSerachNum + 4; i++) {
        if (!keys[i]) break;
        var size = 16 + keyMap[keys[i]] * 2;
        if (!size) size = 26
        size += 'px'
        strHTML += `<span class="keys" data-trans="${keys[i]}" data-value="${keys[i]}" onclick="filterImg(this);setPopularSearch(this)" style="font-size:${size}">${keys[i]}</span>`;
    }
    document.querySelector('.searched-show').innerHTML = strHTML;
}

function setPopularSearch(btn) {
    console.log(btn);
    console.log(btn.style.fontSize);
    var size = btn.style.fontSize
    console.log('size', size);
    var oldSize = size.split('')
    oldSize.pop()
    oldSize.pop()
    size = oldSize.join('')
    size++
    size += 2
    return btn.style.fontSize = size + 'px'
}
